...
(defun rails/webserver/make-server-command (type port env)
  (cond
   ((string= "thin" type)
    (cons type
      ...
      (in-directory root
        (let* ((cmd-alist (rails/webserver/make-server-command type port env))
          ...
          (when (processp proc)
            ...
            (rails/notify (format "%s (%s) starting with port %s"
                                  (capitalize type)
                                  env
                                  port))))))))
...
(defun rails/webserver/run-with-env (&optional env port)
  ...
  (unless (rails/webserver/started-p)
    (when-bind (root (rails/root))
      ...
      (rails/webserver/start root rails/webserver/default-type env port))))
